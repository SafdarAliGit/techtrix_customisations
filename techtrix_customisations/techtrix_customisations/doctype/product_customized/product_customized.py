...
from techtrix_customisations.api import get_serial_nos
from techtrix_customisations.techtrix_customisations.doctype.serial_no import SerialNoItemError
...
class ProductCustomized(Document):
    ...
    def validate_serial_nos(self):
        ...
        if self.type == 'Disassemble':
            if cint(self.qty) != len(serial_nos):
                frappe.throw(("{0} Serial numbers required for Item {1}. You have provided {2}.").format(self.qty,
                                                                                                         self.item_code,
                                                                                                         len(serial_nos)))
        ...
        for d in self.serial_nos:
            ...
            if sr.item_code != self.item_code:
                frappe.throw(
                    _("Row# {0} Serial No {1} does not belong to Item {2}").format(d.idx, d.serial_no, self.item_code),
                    SerialNoItemError)
    ...
    def make_stock_entries(self):
        ...
        if self.type == 'Assemble':
            ...
            for d in self.items:
                serial_no = make_autoname("{0}-.######").format(remove_parentheses(d.item_code))
...
def make_stock_entry(doc):
    ...
    default_expense_account = frappe.get_cached_value("Company", doc.company,
                                                      "default_product_customized_expense_account")
    if not default_expense_account:
        frappe.throw(_('Please set Default Product Customized Expense Account in {0}').format(
            frappe.get_desk_link("Company", doc.company)))
...
def remove_parentheses(input_string):
    ...
    string_without_open_parentheses = input_string.replace('(', '')
    # Replace all occurrences of ')' with an empty string
    string_without_parentheses = string_without_open_parentheses.replace(')', '')
