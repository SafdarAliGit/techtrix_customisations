...
def get_cogs_data(
	...
	for root in frappe.db.sql(
		"""select lft, rgt from tabAccount
			where root_type=%s and name='501 - Cost of Goods Sold'""",
		root_type,
		as_dict=1,
	):
	...
	out = prepare_cogs_data(accounts, balance_must_be, period_list, company_currency)
	out = filter_out_zero_value_rows(out, parent_children_map)

	if out and total:
		add_total_cogs_row(out, root_type, balance_must_be, period_list, company_currency)
...
def get_expenses_data(
	...
	accounts = get_only_expense_accounts(company, root_type)
	if not accounts:
		return None
...
def prepare_cogs_data(accounts, balance_must_be, period_list, company_currency):
	data = []
...
def add_total_cogs_row(out, root_type, balance_must_be, period_list, company_currency):
	total_row = {
		"account_name": _("Total {0} ({1})").format(_("Cost of Goods Sold"), _(balance_must_be)),
...
def get_only_expense_accounts(company, root_type):
	return frappe.db.sql(
		"""
		select name, account_number, parent_account, lft, rgt, root_type, report_type, account_name, include_in_gross, account_type, is_group, lft, rgt
		from `tabAccount`
		where company=%s and root_type=%s and name !='501 - Cost of Goods Sold' order by lft""",
		(company, root_type),
		as_dict=True,
	)
